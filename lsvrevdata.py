...
import lsvrealtime as freal

#from lsvrealtime import masterCodeList
...
#mongodb+srv://devbox72:PyeCYJ0G72MlcpGU@tradeccs.fo3b3.mongodb.net/tradeccs?retryWrites=true&w=majority
...
def stockspurchase_futs(trid,count,data):
    ...
    try:
        ...
        dataDic['tr_id']=trid
        dataDic['code']= chgKospiCode(data['futcode']) #선물 코드
        dataDic['time']=data['chetime'] #영업시간
        dataDic2['tr_id']=trid
        dataDic2['code']= chgKospiCode(data['futcode']) #선물 코드
...
def stockhoka_overseafut(trid,count,tsize,data):
    ...
    try:
        ...
        if tsize == 1.0 and dataDic2['sap_point1'].endswith(".0"):
        ...
        jsonData2 = json.dumps(dataDic2)
        #print(jsonData)
        print(f"해외지수 호가수신 {dataDic['code']}: {dataDic['sap_point1']}")
        sendToServer('tradevolumn',jsonData)
        sendToTrServer('tradevolumn',jsonData2)
        '''
        current_time = datetime.datetime.now() 
...
def stockspurchase_overseafut(trid,count,tsize,data):
    ...
    try:
        ...
        if tsize == 1.0 and dataDic['cpoint'].endswith(".0"):
